...
const OacForm = ({ sac,onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedIssueDate = issueDate.length > 0 ? new Date(issueDate).toISOString().split('T')[0] : null;
    const formattedResolutionDate = resolutionDate.length > 0 ? new Date(resolutionDate).toISOString().split('T')[0] : null;
    ...
    try {
      ...
      await dispatch(createOac({ sacId: sac.id, oacData }));
